...
public enum _SpawnMode{Continous, WaveCleared, RoundBased, SkippableContinous, SkippableWaveCleared}

public class SpawnManager : MonoBehaviour {

	public delegate void WaveStartSpawnHandler(int waveID);
	public static event WaveStartSpawnHandler WaveStartSpawnE;

	public delegate void WaveSpawnedHandler(int waveID);
	public static event WaveSpawnedHandler WaveSpawnedE;
	
	public delegate void WaveClearedHandler(int waveID);
	public static event WaveClearedHandler WaveClearedE;
	
	public delegate void WaveClearedForSpawningHandler(bool flag);
	public static event WaveClearedForSpawningHandler ClearForSpawningE;
	...
	void Start () {
		...
		//~ waypoints=new List<Vector3>();
			//~ foreach(Transform point in path.waypoints){
			//~ waypoints.Add(point.position);
		//~ }
		
		//listen to event
		Unit.DeadE += CheckIsWaveCleared;
		Unit.ScoreE += CheckIsWaveCleared;
	...
	public bool _Spawn(){
		if(isClearForSpawning && GameControl.gameState!=_GameState.Ended){
			...
			if(currentWave>=waves.Length){
				...
				if(spawnMode==_SpawnMode.Continous || spawnMode==_SpawnMode.SkippableContinous){
	...
	void CheckIsWaveCleared(int waveID){
		...
		if(waves[waveID].spawned && waves[waveID].activeUnitCount==0){
			...
			if(WaveClearedE!=null) WaveClearedE(waveID);
			...
			if(spawnMode==_SpawnMode.RoundBased && currentWave<waves.Length){
				...
				ClearForSpawningE(isClearForSpawning);
			}
		}
	}
	...
	public void SpawnWave(){
		...
		currentWave+=1;
		if(WaveStartSpawnE!=null) WaveStartSpawnE(currentWave+1);
	}
	...
	IEnumerator CheckSpawn(int waveID){
		...
		if(currentWave<waves.Length){
			...
			if(ClearForSpawningE!=null) ClearForSpawningE(isClearForSpawning);
		}
	}
	...
	static public float TimeNextSpawn(){
